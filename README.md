...
# RDB
> RDB全程redis数据备份文件，也简称为redis数据快照，简单来说就是把内存中的所有数据都记录到磁盘中。当redis实例故障重启后，
> 从磁盘读取快照文件，恢复数据。
> redis停机是会执行一次RDB。
...
AOF成为Append Only File .Redis处理的每一个写命令都会记录在AOF文件，可以看作是命令日志文件
...
AOF
AOF文件会比RDB文件大得多。而且AOF会记录对同一个key的多吃写操作，但只有最后一次写操作才有意义。通过bgrewriteaof命令，可以让AOF文件执行重写
```
...
> Replication Id：是数据集的标记，id一致则说明是同一数据集。每一个master都有唯一的replid，slave则会继承master节点的replid
> offset：偏移量，随着记录在repl_baklog中的数据增多则逐渐增大。slave完成同步时也会记录当前同步的offset。如果slave的offset小于master的offset，说明
> slave数据落后于master，需要更新。
...
Sentinel基于心跳机制监控服务状态，每隔1秒向集群的每隔实例发送ping命令
* 主观下线：如果某sentinel节点发现某实例未在规定时间响应
* 客观下线：超过指定数量的sentinel都认为该实例主管下线
...
```
redis-cli --cluster help 提供了很多集群命令
```
...
* takeover：直接执行第5歩，忽略数据一致性、忽略master状态和其它master的意
